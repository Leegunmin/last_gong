...
public class MemberDataAdapter_eval extends BaseAdapter {

    ArrayList<MemberData_eval> datas;
    LayoutInflater inflater;

    public MemberDataAdapter_eval(LayoutInflater inflater, ArrayList<MemberData_eval> datas) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if( convertView==null){
            ...
            convertView= inflater.inflate(R.layout.list_row_eval, null);
        }
        ...
        RatingBar rating_eval_list = (RatingBar)convertView.findViewById(R.id.rating_eval_list);

        TextView writer_eval_list= (TextView)convertView.findViewById(R.id.writer_eval_list);
        TextView star_eval_list= (TextView)convertView.findViewById(R.id.star_eval_list);

        TextView date_eval_list= (TextView)convertView.findViewById(R.id.date_eval_list);
        TextView review_eval_list= (TextView)convertView.findViewById(R.id.review_eval_list);
        //현재 position( getView()메소드의 첫번재 파라미터 )번째의 Data를 위 해당 View들에 연결..

        float rating = (float) datas.get(position).getStar() / (float) 2;
        rating_eval_list.setRating(rating);
        writer_eval_list.setText( datas.get(position).getWriter() );
        star_eval_list.setText( rating + " / 5" );

        date_eval_list.setText( String.valueOf(datas.get(position).getDat()));
        review_eval_list.setText( datas.get(position).getReview() );
